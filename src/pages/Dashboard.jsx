...
export default function Dashboard() {
  ...
  const fetchTransactions = (
    pageNum = 1,
    shouldAppend = false,
    currentStatus = showScheduled ? "pending" : "paid",
  ) => {
  ...
  const checkPending = () => {
    ...
    if (!token) return;
    api
      .get("/api/transactions?status=pending&limit=1", {
        headers: { Authorization: `Bearer ${token}` },
      })
    ...
    <div className="min-h-screen p-8 bg-gray-100">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {/* Income Card */}
          <div
            className={`p-4 rounded-lg shadow transition-colors ${activeFilter === "income" ? "bg-blue-600 text-white" : "bg-white"}`}
          >
            <h3
              className={
                activeFilter === "income" ? "text-blue-100" : "text-gray-500"
              }
            >
              Income
            </h3>
            <p
              className={`text-2xl font-bold ${activeFilter === "income" ? "text-white" : "text-green-600"}`}
            >
              R$ {income.toFixed(2)}
            </p>
          </div>
          {/* Expenses Card */}
          <div
            className={`p-4 rounded-lg shadow transition-colors ${activeFilter === "expense" ? "bg-blue-600 text-white" : "bg-white"}`}
          >
            <h3
              className={
                activeFilter === "expense" ? "text-blue-100" : "text-gray-500"
              }
            >
              Expenses
            </h3>
            <p
              className={`text-2xl font-bold ${activeFilter === "expense" ? "text-white" : "text-red-600"}`}
            >
              R$ {expense.toFixed(2)}
            </p>
          </div>
          {/* Total Balance Card */}
          <div
            className={`p-4 rounded-lg shadow transition-colors ${activeFilter === "all" ? "bg-blue-600 text-white" : "bg-white"}`}
          >
            <h3
              className={
            ...
            <p
              className={`text-2xl font-bold ${activeFilter === "all" ? "text-white" : "text-gray-800"}`}
            >
              R$ {total.toFixed(2)}
            </p>
          </div>
          ...
          >
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <button
                  type="submit"
                  className="px-4 py-2 text-white bg-blue-600 rounded hover:bg-blue-700 flex-1"
                >
                          ...
                          <div className="flex justify-between flex-1 mr-4">
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-gray-500 font-medium">
                                {formatDate(
                                  t.date || t.createdAt || t.created_at,
                                )}{" "}
                                •{" "}
